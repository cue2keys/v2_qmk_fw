...
static uint16_t cpi_setting[NUM_MODULAR_ADNS5050];
...
bool modular_adns5050_probe(uint8_t index) {
    ...
    modular_adns5050_wake_up(index, false);
...
bool modular_adns5050_is_connected(uint8_t index) {
    if (index >= NUM_MODULAR_ADNS5050) {
        return false;
    }
    return connected[index];
...
uint8_t modular_adns5050_get_connected_flags(uint8_t *out, uint8_t max) {
    if (out == NULL || max == 0) {
        return 0;
    }

    uint8_t count = NUM_MODULAR_ADNS5050;
    if (count > max) {
        count = max;
    }
...
void modular_adns5050_wake_up(uint8_t index, bool connected_only) {
    ...
    modular_adns5050_set_cpi(index, cpi_setting[index] ? cpi_setting[index] : ADNS5050_DEFAULT_CPI);
}
...
void modular_adns5050_set_cpi(uint8_t index, uint16_t cpi) {
    uint16_t clamped = constrain(cpi, ADNS5050_CPI_MIN, ADNS5050_CPI_MAX);
    uint8_t  cpival  = constrain((clamped / ADNS5050_CPI_INTERVAL), 0x1, 0xD);

    cpi_setting[index] = (uint16_t)(cpival * ADNS5050_CPI_INTERVAL);
    if (powered_down[index]) {
        return;
    }
...
uint16_t modular_adns5050_get_cpi(uint8_t index) {
    ...
    return (uint16_t)((cpival & 0x0F) * ADNS5050_CPI_INTERVAL);
}
...
report_mouse_t modular_adns5050_get_report(uint8_t index, report_mouse_t mouse_report) {
    if (!connected[index] || powered_down[index]) {
        return mouse_report;
    }
